...
新导出的日志、视频和 replay 默认保持本地产物；`.gitignore` 会排除它们。已核验并公开的 22 份胜局 replay 单独保存在 `media/replays/`，3 部完整胜局视频收录在 `media/videos/`，并在 README 中使用 GitHub 原生播放器直接播放；原始高码率录像保留在 Releases，详见[中英双语媒体清单](../media/README.md)。清单包含版本、胜局区间和 SHA-256，原始 replay 未改写。论文中的少量截图和固定图表同样属于明确保留的研究材料。
