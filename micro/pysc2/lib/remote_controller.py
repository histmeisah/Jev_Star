...
from s2clientprotocol import common_pb2 as co_pb
...
class RemoteController(object):
  ...
  def observe(self, disable_fog=False, target_game_loop=0):
    ...
    # Query controllable units only; the game cannot accept queries after ending.
    available_abilities = []
    unit_tags = [unit.tag for unit in obs.observation.raw_data.units
                 if unit.alliance == 1 and unit.health > 0]
    if unit_tags and not obs.player_result:
      query = q_pb.RequestQuery()
      for tag in unit_tags:
        ability_query = query.abilities.add()
        ability_query.unit_tag = tag
      available_abilities = self._client.send(query=query).abilities
    ...
    return obs, available_abilities
